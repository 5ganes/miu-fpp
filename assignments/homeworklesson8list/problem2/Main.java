...
public class Main {
	...
	public static List<EmployeeData> combine(List<Staff> staff, List<Teacher> teachers) {
		ArrayList<EmployeeData> empData = new ArrayList<>();
		empData.addAll(staff); // for (Staff s : staff) empData.add(s); can also be used
		empData.addAll(teachers); // for (Teacher t : teachers) empData.add(t); can also be used
		return empData;
	}
